...
export interface RosterCardView {
  /** Stable filename identity; display names can change without renaming files. */
  slug?: string
  heading: string
  name: string
  age: number
...
export interface BrainsList {
  current: string
  models: BrainOpt[]
  veniceBudget?: { limitUsd: number; accountedUsd: number }
}
...
export interface ChatMsg {
  /** Stable identity from the gateway, inbox manifest, or a Terrarium client UUID. */
  id: string
  role: 'user' | 'assistant'
  text: string
  ts: number | null
  /** Present for locally-authored messages until delivery has settled. */
...
export interface ChatStatus {
  state: 'connecting' | 'reconnecting' | 'ready' | 'sending' | 'closed' | 'error'
  detail: string
}
export interface CharActivateResult { ok: boolean; message: string }
export interface CharDeleteResult { ok: boolean; message: string; roster?: RosterView }

export interface ChatSendRequest {
  /** Generated in the renderer and retained for retries; also used as gateway idempotency key. */
  id: string
  text: string
}
