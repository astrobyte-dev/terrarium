...
export function buildDraftPrompt(seed: DraftSeed): string {
  ...
  return [
    ...
    'Give her a SPECIFIC ethnicity/nationality and reflect it concretely in both "look" and the photo "identity" (skin tone, hair, features). Vary it — pick something that fits the concept, do not default to a generic white woman.',
    '',
    'Output ONLY a JSON object (no prose, no code fences) with EXACTLY these keys:',
    '- look: short physical description INCLUDING her ethnicity (comma phrases)',
    ...
    '- photo: object with identity (ethnicity + adult face/body descriptors incl. the age), outfit (default clothing), shot (camera framing)',
    '',
    'Return the JSON now.',
  ].join('\n')
}
